...
export default function HeaderSection({ block }: any) {
  return (
    <header id="top">
      {/* Info Section */}
      <div className="w-full flex-col-reverse md:flex-row flex gap-4 md:gap-6">
        {/* Image */}
        {block.avatar && (
          <img
            src={block.avatar}
            alt="Geraldine Ragsac"
            className="size-40 rounded-full mx-auto md:mx-0 mb-4 md:mt-4"
          />
        )}
        {/* Content */}
        <div className="grow text-center md:text-left mt-5">
          <h1 className="text-4xl">{block.title}</h1>
          <h2 className="text-2xl">{block.subtitle}</h2>
          <p className="mt-2">{block.blurb}</p>
        </div>
      </div>

      <div className="flex gap-4 md:gap-6">
        <CTAAnchor href={block.resume} variant="primary" className="grow">
          Download Resume
        </CTAAnchor>
        <CTAAnchor
          href={`https://linkedin.com/in/${block.linkedinProfile}`}
          variant="secondary"
          className="w-fit!"
        >
          <svg
        ...
        <CTAAnchor
          href={`https://github.com/${block.githubUsername}`}
          variant="secondary"
          className="w-fit!"
        >
          <svg
